...
export class MyGoban extends Component {
    constructor(props) {
        ...
        this.view_window = props.view_window || {x: [0,18], y: [0,18]};
        var win = this.view_window;
        this.view_window['y_size'] = win.y[1]-win.y[0];
        this.view_window['x_size'] = win.x[1]-win.x[0];

        var max_dim = Math.max(this.view_window.x_size, this.view_window.y_size);

        this.cellSize = this.boardSize / max_dim;
    ...
    boardToSVG(coordinate) {
        return {
            x: this.startPoint + (coordinate.x * this.cellSize) - (this.view_window.x[0] * this.cellSize),
            y: this.startPoint + (coordinate.y * this.cellSize) - (this.view_window.y[0] * this.cellSize),
        };
    }

    svgToBoard(x, y) {
        const coordinate = new go.Coordinate(
            round((x - this.startPoint) / this.cellSize) + (this.view_window.x[0]),
            round((y - this.startPoint) / this.cellSize) + (this.view_window.y[0]),
        );
    ...
    render() {
        //const svgViewBox = `${9*this.cellSize} 0 ${this.viewBox} ${this.viewBox - 9*this.cellSize}`;
        ...
        // TODO this code is broken if you want a different region
        // it's supposed to suppress drawing the enclosing line in a region
        // where a full size board would continue...
        var x_range, y_range;
        if (this.view_window.x[0] > 0) {
          x_range = [1,this.view_window.x_size];
        } else {
          x_range = [0,this.view_window.x_size];
        }

        if (this.view_window.y[0] > 0) {
          y_range = [1,this.view_window.y_size];
        } else {
          y_range = [0,this.view_window.y_size];
        }
        ...
        return (
            <svg viewBox={svgViewBox} onClick={this.handleClick.bind(this)}>
                ...
                {range(x_range[0], x_range[1]).map(index =>
                    <line key={'x' + index}
                ...
                {range(y_range[0], y_range[1]).map(index =>
                    <line key={'y' + index}
